...
useEffect(()=>{
    const getTwits = async()=>{
        try {
          const querySnapshot = await getDocs(query(collection(db, "Twits"),where("id", "==", idU)))  
          const docs = []
    ...
    const getTwits1 = async()=>{
        try {
          const querySnapshot = await getDocs(query(collection(db, "Twits"),where("id", "==", idU),where("cont", "==", 1)))  
          const docs = []
          querySnapshot.forEach((doc)=>{
              docs.push({...doc.data(),id:doc.id})
          })
